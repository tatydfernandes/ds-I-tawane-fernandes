...
public String inserir(CarroBean carro){
    String sql = "insert into carro(placa,cor,descricao,proprietario) values (?,?,?,?)";
    try{
...
public String alterar(CarroBean carro){
    String sql = "update carro set cor = ?, descricao = ?, proprietario = ?";
        sql += "where placa = ?";
    ...
    try{
        PreparedStatement ps = getCon().prepareStatement(sql);
            ...
            ps.setString(3, carro.getProprietario());
